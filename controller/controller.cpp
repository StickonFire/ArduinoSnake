...
void ConstrainedVectorToArray::convertConstrainedVector(ConstrainedVector<ConstrainedVector<MapTileState,ledCols>,ledRows> &input, bool output[ledRows][ledCols]){
    ...
    for(int i = 0; i < ledRows; i++){
        for(int j = 0; j < ledCols; j++){
            if(i >= screenMinX && i < screenMaxX && j >= screenMinY && j < screenMaxY)
                output[i][j] = ((input[i-screenMinX][j-screenMinY] == Empty) ? false : true);
            else
                output[i][j] = true;
        }
    }
